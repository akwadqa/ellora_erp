...
def _execute(filters, additional_table_columns=None):
	...
	for inv in invoice_list:
		...
		row = {
			"voucher_type": "Sales Return" if inv.is_return else inv.doctype,
			"voucher_no": inv.name,
			"posting_date": inv.posting_date,
			"customer": inv.customer,
			"customer_name": inv.custom_cash_customer_name if inv.customer == "Cash Customer" else inv.customer_name,
...
def get_invoices(filters, additional_query_columns):
	...
	if not filters.get("include_intercompany_sales"):
		query = query.where(si.is_internal_customer == 0)
